...
async function main() {
  ...
  const cwd = path.resolve(rootDir, "experiences", "sandbox", "fact_observe");
  await rm(cwd, { recursive: true, force: true });
  await mkdir(cwd, { recursive: true });

  console.log("================================================================");
  console.log("🔍 QUAN SÁT THỰC NGHIỆM PROMPT 'fact.js'");
  console.log(`📂 Sandbox CWD: ${cwd}`);
  console.log(`⚙️ Config: temp=0, repeat_penalty=1.0 (chuẩn trung tính)`);
  console.log("================================================================\n");
  ...
  while (turn < maxTurns) {
    ...
    if (!choice) {
      console.error("❌ Không nhận được choice từ model.");
      break;
    }
    ...
    if (!msg.tool_calls || msg.tool_calls.length === 0) {
      console.log(`\n🏁 [Turn ${turn}] Model đã kết thúc và không gọi thêm tool.`);
      break;
    }
    ...
    for (const toolCall of msg.tool_calls) {
      ...
      console.log(`   📥 [Tool Result Output]:\n   ${toolResult.slice(0, 400)}${toolResult.length > 400 ? "..." : ""}`);
