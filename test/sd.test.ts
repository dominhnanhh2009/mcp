...
test("buildSdToolDescription indicates model name and SD1.5 base", () => {
...
test("gen_image tool execution returns base64 image content and saves file to workspace", async () => {
  ...
  const result = (await tool.handler(
    {
      ...
      width: 512,
      height: 512,
      sampling: { scheduler: "discrete", method: "euler_a", steps: 20 },
...
test("server dynamically includes gen_image when sd-server is online and excludes it when offline", async () => {
  const tempWorkspace = await mkdtemp(path.join(tmpdir(), "mcp-sd-test-"));
  
  // Test server with online mock sd-server
  ...
  assert.ok(onlineTools.tools.some((t) => t.name === "gen_image"));
